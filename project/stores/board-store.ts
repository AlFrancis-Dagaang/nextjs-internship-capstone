...
export const useBoardStore = create<BoardState>((set, get) => ({
  ...
  addTask: (listId, task) =>
    set((s) => ({
      lists: s.lists.map((l) => {
        // Strip the task from any list it might currently be lingering in
        const filteredTasks = l.tasks.filter((t) => t.id !== task.id)
        if (l.id !== listId) return { ...l, tasks: filteredTasks }
        // Add it fresh to the target list
  ...
  dragOver: (activeId, overId) => {
    ...
    set((s) => {
      ...
      if (sourceList.id === destList.id) {
        ...
        const reordered = arrayMove(sourceList.tasks, currentIndex, insertIndex)
        return {
          lists: s.lists.map((l) =>
